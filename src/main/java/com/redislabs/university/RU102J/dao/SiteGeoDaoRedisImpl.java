...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             // START Challenge #5
             List<GeoRadiusResponse> radiusResponses = performGeoRadiusQuery(coord, radius, radiusUnit, jedis);
             // END Challenge #5
    ...
    private List<GeoRadiusResponse> performGeoRadiusQuery(Coordinate coord, Double radius, GeoUnit radiusUnit, Jedis jedis) {
        return jedis.georadius(RedisSchema.getSiteGeoKey(), coord.getLng(),
